...
export async function fetchCardData() {
  ...
  try {
    ...
    // Upstream reads data[0].count, which is postgres.js's *row count* from the
    // command tag (always 1 here), not the COUNT(*) column. The shim in db.ts
    // returns plain arrays, so the column is read explicitly.
